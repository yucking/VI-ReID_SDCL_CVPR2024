...
"""
SDCL + metric-stage2soft v4_push_v2_memsafe

Loss 公式完全沿用源码 trainers，不添加任何额外 loss 项。
创新点在主脚本的 stage2 伪标签管线中：

CGLF：在 CRA + soft structure smooth 之后，根据 CRA 置信度
（intersect_count / TOPK2）过滤低置信度样本，将其标记为 -1
排除出训练集，防止噪声伪标签通过 shared memory 污染梯度。

参数：
  --cglf-threshold  CRA 置信度阈值，低于此值的 RGB 样本排除出 stage2 训练（默认 0.0）
  --cross-modal-mode  跨模态邻域模式（默认 alternating，与源码一致）

在 v4_push 基础上仅加入后期 tailtrim decay：
  [TAIL] epoch=... start=... warmup=... decay_after=... decay_to=... pct=... effective_pct=... filtered=.../...
"""
import argparse
...
from clustercontrast.models.reciprocal_ambiguity_memory import ReciprocalAmbiguityMemory
...
############## 共享层
            ...
            if getattr(args, 'enable_ras', False):
                ambiguity_mask, edge_count, valid_count, density, mean_similarity = build_reciprocal_ambiguity_mask(
                    features_rgb_ori, pseudo_labels_rgb, features_rgb_s,
                    features_ir_ori, pseudo_labels_ir, features_ir_s,
                    num_cluster_ir,
                    topk=int(getattr(args, 'ras_topk', 2)),
                    margin=float(getattr(args, 'ras_margin', 0.03)),
                )
                ras_memory = ReciprocalAmbiguityMemory(
                    768, num_cluster_ir, ambiguity_mask, temp=args.temp,
                    momentum=0.1, use_hard=args.use_hard).cuda()
                ras_memory.features.copy_(shared_memory.features)
                ras_memory_s = ReciprocalAmbiguityMemory(
                    768, num_cluster_ir, ambiguity_mask, temp=args.temp,
                    momentum=0.1, use_hard=args.use_hard).cuda()
                ras_memory_s.features.copy_(shared_memory_s.features)
                trainer.memory_ir = ras_memory
                trainer.memory_rgb = ras_memory
                trainer.memory_rgb_s = ras_memory_s
                trainer.memory_ir_s = ras_memory_s
                print('[RAS] epoch={} labels={} edges={} density={:.4%} mean_cross_sim={:.4f} topk={} margin={:.3f}'.format(
                    epoch, valid_count, edge_count, density, mean_similarity,
                    int(getattr(args, 'ras_topk', 2)), float(getattr(args, 'ras_margin', 0.03))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--enable-ras', action='store_true',
                        help='Mask reciprocal cross-modal ambiguity pairs from stage2 memory negatives.')
    parser.add_argument('--ras-topk', dest='ras_topk', type=int, default=2,
                        help='Reciprocal prototype neighbors used by RAS.')
    parser.add_argument('--ras-margin', dest='ras_margin', type=float, default=0.03,
                        help='Maximum similarity gap from the current cross-modal prototype match for RAS.')
    main()
