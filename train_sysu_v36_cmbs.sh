...
LOG_DIR="${LOG_DIR:-${LOG_ROOT}/sysu_v36_cmbs_seed${SEED}_fullchain}"
WORKERS="${WORKERS:-8}"
EPOCHS="${EPOCHS:-50}"
...
ENABLE_CMBS="${ENABLE_CMBS:-1}"
CMBS_NUM_INSTANCES="${CMBS_NUM_INSTANCES:-8}"
CMBS_MIN_COMMON_IDS="${CMBS_MIN_COMMON_IDS:-64}"
CMBS_PAIR_WEIGHT="${CMBS_PAIR_WEIGHT:-0.20}"
CMBS_PAIR_TAU="${CMBS_PAIR_TAU:-0.07}"
...
if [ "${ENABLE_CMBS}" = "1" ]; then
    extra_args+=(
        --enable-cmbs
        --cmbs-num-instances "${CMBS_NUM_INSTANCES}"
        --cmbs-min-common-ids "${CMBS_MIN_COMMON_IDS}"
        --cmbs-pair-weight "${CMBS_PAIR_WEIGHT}"
        --cmbs-pair-tau "${CMBS_PAIR_TAU}"
    )
fi

if [ -n "${STAGE2_RESUME_FULL_STATE}" ]; then
    echo "==> v36 mode: full-state Stage-2 resume"
elif [ "${STAGE2_ONLY}" = "1" ]; then
    echo "==> v36 mode: model-only Stage-2 compatibility"
else
    echo "==> v36 mode: full-chain from epoch 0"
fi
echo "==> v36 CMBS: enable=${ENABLE_CMBS} inst=${CMBS_NUM_INSTANCES} min_common=${CMBS_MIN_COMMON_IDS} pair_weight=${CMBS_PAIR_WEIGHT} tau=${CMBS_PAIR_TAU}"

python sdcl_sysu_v36_cmbs.py \
    -b 96 \
    -j "${WORKERS}" \
    -a agw \
    -d sysu_all \
