...
"""
SDCL v37 S1 handoff: Stage-1 adaptive transition.

v36 showed that adding hard cross-modal Stage-2 positives can optimize its own
loss while degrading SYSU metrics. This version moves the main closure upstream:
Stage-1 is allowed to run longer, the Stage-2 handoff checkpoint is selected only
from a late Stage-1 window, and the selector can emphasize mINP-heavy retrieval
quality instead of reusing the legacy mAP-dominant handoff rule.

Primary knobs:
  --cmlabel                     Stage-2 start epoch; v37 script defaults to 36.
  --stage1-select-start         Earliest epoch allowed to become 20model_best.
  --stage1-best-select-mode     Add tail/paper modes for handoff selection.
  --stage1-save-candidates      Optional checkpoint snapshots for handoff audit.
"""
import argparse
import csv
...
def main():
    ...
    cfg.freeze()
    if args.stage1_select_start < 0:
        parser.error('--stage1-select-start must be >= 0.')
    if args.stage1_select_start >= args.cmlabel:
        parser.error('--stage1-select-start must be smaller than --cmlabel.')
...
def main_worker_stage2(args,log_name):
    ...
    print('==> stage1-handoff: select_window=[{}, {}) save_candidates={}'.format(
        int(getattr(args, 'stage1_select_start', 0)),
        int(trainer.cmlabel),
        bool(getattr(args, 'stage1_save_candidates', False))))
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch == trainer.cmlabel) :# Enter original Stage-2 at the configured epoch.
            # Load the best late-window Stage-1 checkpoint saved before Stage-2.
            stage1_checkpoint_path = osp.join(args.logs_dir, '20model_best.pth.tar')
            if not osp.isfile(stage1_checkpoint_path):
                raise RuntimeError(
                    'Missing Stage-1 handoff checkpoint {}. Check --stage1-select-start < --cmlabel '
                    'and that evaluation ran before Stage-2.'.format(stage1_checkpoint_path)
                )
            checkpoint = load_checkpoint(stage1_checkpoint_path)
            print('[S1-HANDOFF] enter Stage-2 at epoch {} from {} | saved_epoch={} '
                  'stage1_score={:.4f} selector={} window=[{}, {}) all_mAP={:.2%} '
                  'all_mINP={:.2%} indoor_mAP={:.2%} indoor_mINP={:.2%}'.format(
                      epoch, stage1_checkpoint_path, checkpoint.get('epoch', '?'),
                      float(checkpoint.get('best_score', 0.0)),
                      getattr(args, 'stage1_best_select_mode', 'legacy'),
                      int(getattr(args, 'stage1_select_start', 0)),
                      int(trainer.cmlabel),
                      float(checkpoint.get('allsearch_mAP', 0.0)),
                      float(checkpoint.get('allsearch_mINP', 0.0)),
                      float(checkpoint.get('indoor_mAP', 0.0)),
                      float(checkpoint.get('indoor_mINP', 0.0))))
...
#################################
            ...
            all_rank1 = float(allsearch_cmc[0])
            indoor_rank1 = float(indoor_cmc[0])

            def score_for_mode(mode):
                if mode == 'tail':
                    return (
                        0.30 * float(allsearch_mAP)
                        + 0.20 * float(indoor_mAP)
                        + 0.20 * float(allsearch_mINP)
                        + 0.30 * float(indoor_mINP)
                    )
                if mode == 'paper':
                    return (
                        0.25 * float(allsearch_mAP)
                        + 0.30 * float(indoor_mAP)
                        + 0.20 * float(allsearch_mINP)
                        + 0.25 * float(indoor_mINP)
                    )
                if mode == 'full':
                    return (
            ...
            stage1_window_start = int(getattr(args, 'stage1_select_start', 0))
            stage1_in_window = (epoch >= stage1_window_start) and (epoch < trainer.cmlabel)
            if stage1_in_window:
            ...
            if epoch < trainer.cmlabel:
                stage1_report = osp.join(args.logs_dir, 'stage1_handoff_candidates.csv')
                write_header = not osp.isfile(stage1_report)
                with open(stage1_report, 'a', newline='') as f:
                    writer = csv.writer(f)
                    if write_header:
                        writer.writerow([
                            'epoch', 'in_handoff_window', 'is_handoff_best', 'selector',
                            'stage1_score', 'best_stage1_epoch', 'best_stage1_score',
                            'all_rank1', 'all_mAP', 'all_mINP',
                            'indoor_rank1', 'indoor_mAP', 'indoor_mINP',
                            'ir_clusters', 'rgb_clusters', 'ir_outlier', 'rgb_outlier'
                        ])
                    writer.writerow([
                        int(epoch), int(stage1_in_window), int(stage1_is_best), stage1_sel,
                        '{:.8f}'.format(float(stage1_score)), int(best_stage1_epoch),
                        '{:.8f}'.format(float(best_stage1_score)),
                        '{:.8f}'.format(all_rank1), '{:.8f}'.format(float(allsearch_mAP)),
                        '{:.8f}'.format(float(allsearch_mINP)), '{:.8f}'.format(indoor_rank1),
                        '{:.8f}'.format(float(indoor_mAP)), '{:.8f}'.format(float(indoor_mINP)),
                        int(num_cluster_ir), int(num_cluster_rgb), int(ir_outlier), int(rgb_outlier)
                    ])
                if stage1_in_window and bool(getattr(args, 'stage1_save_candidates', False)):
                    candidate_path = osp.join(args.logs_dir, 'stage1_candidate_epoch_{:03d}.pth.tar'.format(epoch))
                    save_checkpoint({
                        'state_dict': model.state_dict(),
                        'epoch': epoch + 1,
                        'stage1_score': float(stage1_score),
                        'selector': stage1_sel,
                        'allsearch_rank1': all_rank1,
                        'allsearch_mAP': float(allsearch_mAP),
                        'allsearch_mINP': float(allsearch_mINP),
                        'indoor_rank1': indoor_rank1,
                        'indoor_mAP': float(indoor_mAP),
                        'indoor_mINP': float(indoor_mINP),
                        'ir_clusters': int(num_cluster_ir),
                        'rgb_clusters': int(num_cluster_rgb),
                        'ir_outlier': int(ir_outlier),
                        'rgb_outlier': int(rgb_outlier),
                    }, False, fpath=candidate_path)
            ...
            if epoch < trainer.cmlabel:
                save_checkpoint10({
                    ...
                    'selector': stage1_sel,
                    'stage1_select_start': int(stage1_window_start),
            ...
            print('\n * Finished epoch {:3d}  sel={}  all_mAP: {:5.1%} indoor_mAP: {:5.1%} '
                  ...
                  's1_window={} s1_best@epoch={} s1_score={:.4f}{}\n'.format(
                      ...
                      int(stage1_in_window), best_stage1_epoch if best_stage1_epoch >= 0 else '-',
                      float(stage1_score), ' S1*' if stage1_is_best else ''))
############################
        lr_scheduler.step()
        # if epoch >30:
        #     break
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default='legacy',
        choices=('legacy', 'full', 'tail', 'paper', 'follow'),
        help='Select 20model_best before cmlabel for Stage-2 warm-start: legacy is mAP-heavy; full matches final selection; tail emphasizes all/indoor mINP; paper balances paper-critical mAP/mINP; follow mirrors --best-select-mode.',
    )
    parser.add_argument('--stage1-select-start', type=int, default=0,
                        help='Earliest epoch allowed to become 20model_best. Use this to force a late Stage-1 handoff window such as [28, cmlabel).')
    parser.add_argument('--stage1-save-candidates', action='store_true',
                        help='Save every Stage-1 checkpoint inside the handoff window for offline audit. This is storage-heavy.')
