...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_memory_under_test', _MODULE_PATH)
_GPRD_MEMORY = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _GPRD_MEMORY
_SPEC.loader.exec_module(_GPRD_MEMORY)

GPRDRelation = _GPRD_MEMORY.GPRDRelation
build_gprd_teacher = _GPRD_MEMORY.build_gprd_teacher
gprd_ranking_loss = _GPRD_MEMORY.gprd_ranking_loss
...
def test_gprd_loss_is_finite_and_backpropagates_with_candidates():
    ...
    loss, stats = gprd_ranking_loss(
        ...
        temperature=0.05, margin=0.10, pairwise_weight=0.20)
    loss.backward()
