...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_replay_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_replay_memory_under_test', _MODULE_PATH)
_gprd_replay_memory = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _gprd_replay_memory
_SPEC.loader.exec_module(_gprd_replay_memory)

GPRDRelation = _gprd_replay_memory.GPRDRelation
build_gprd_teacher = _gprd_replay_memory.build_gprd_teacher
merge_gprd_teacher_cache = _gprd_replay_memory.merge_gprd_teacher_cache
gprd_ranking_loss = _gprd_replay_memory.gprd_ranking_loss


def test_gprd_replay_teacher_keeps_stable_reciprocal_edges():
...
def test_gprd_replay_loss_is_zero_without_active_candidates():
...
def test_gprd_replay_cache_carries_previous_train_edges():
...
def test_gprd_replay_loss_is_finite_and_backpropagates_with_candidates():
    relation = GPRDRelation(
...
def test_gprd_replay_loss_filters_conflicts_and_reports_adaptive_scales():
    relation = GPRDRelation(
