...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_guard_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_guard_memory_under_test', _MODULE_PATH)
_gprd_guard_memory = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _gprd_guard_memory
_SPEC.loader.exec_module(_gprd_guard_memory)

GPRDRelation = _gprd_guard_memory.GPRDRelation
build_gprd_teacher = _gprd_guard_memory.build_gprd_teacher
gprd_ranking_loss = _gprd_guard_memory.gprd_ranking_loss
...
if __name__ == '__main__':
    ...
    print('GPRD-GUARD smoke test passed')
