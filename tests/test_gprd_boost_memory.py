...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_boost_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_boost_memory_under_test', _MODULE_PATH)
_gprd_boost_memory = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _gprd_boost_memory
_SPEC.loader.exec_module(_gprd_boost_memory)

GPRDRelation = _gprd_boost_memory.GPRDRelation
build_gprd_teacher = _gprd_boost_memory.build_gprd_teacher
gprd_ranking_loss = _gprd_boost_memory.gprd_ranking_loss


def test_gprd_boost_teacher_keeps_stable_reciprocal_edges():
...
def test_gprd_boost_loss_is_zero_without_active_candidates():
...
def test_gprd_boost_loss_is_finite_and_backpropagates_with_candidates():
    relation = GPRDRelation(
...
def test_gprd_boost_loss_filters_conflicts_and_reports_adaptive_scales():
    relation = GPRDRelation(
