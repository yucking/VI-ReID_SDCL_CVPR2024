...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_plg_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_plg_memory_under_test', _MODULE_PATH)
_gprd_plg_memory = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _gprd_plg_memory
_SPEC.loader.exec_module(_gprd_plg_memory)

GPRDRelation = _gprd_plg_memory.GPRDRelation
build_gprd_teacher = _gprd_plg_memory.build_gprd_teacher
merge_gprd_teacher_cache = _gprd_plg_memory.merge_gprd_teacher_cache
gprd_ranking_loss = _gprd_plg_memory.gprd_ranking_loss


def test_GPRD_FORCE_teacher_keeps_stable_reciprocal_edges():
...
def test_GPRD_PLG_teacher_uses_pseudo_label_positive_guard():
    torch.manual_seed(17)
    centers = F.normalize(torch.randn(3, 8), dim=1)
    rgb = F.normalize(centers + 0.01 * torch.randn(3, 8), dim=1)
    # Make raw nearest order tempting but pseudo labels should decide positives.
    ir = F.normalize(torch.stack((centers[1], centers[0], centers[2])) +
                     0.01 * torch.randn(3, 8), dim=1)
    cams = torch.tensor([0, 1, 1])
    labels = torch.tensor([0, 1, 2])
    ir_labels = torch.tensor([1, 0, 2])

    rgb_to_ir, _, metrics = build_gprd_teacher(
        rgb, ir,
        rgb_view2=rgb,
        ir_view2=ir,
        rgb_labels=labels,
    ...
    assert metrics['label_positive_rate'] == 1.0
    for row in range(3):
        pos_id = int(rgb_to_ir.candidate_ids[row, 0].item())
        assert int(ir_labels[pos_id].item()) == int(labels[row].item())


def test_GPRD_FORCE_loss_is_zero_without_active_candidates():
...
def test_GPRD_FORCE_cache_carries_previous_train_edges():
...
def test_GPRD_FORCE_loss_filters_low_confidence_rows():
    relation = GPRDRelation(
...
def test_GPRD_FORCE_loss_is_finite_and_backpropagates_with_candidates():
    relation = GPRDRelation(
...
def test_GPRD_FORCE_loss_reports_explicit_gap_terms():
    relation = GPRDRelation(
        candidate_ids=torch.tensor([[0, 1, 2]], dtype=torch.long),
...
def test_GPRD_FORCE_loss_filters_conflicts_and_reports_adaptive_scales():
    relation = GPRDRelation(
        candidate_ids=torch.tensor([[0, 1, 2]], dtype=torch.long),
        teacher_probs=torch.tensor([[0.70, 0.20, 0.10]]),
        positive_mask=torch.tensor([[True, False, False]]),
