...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_soft_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_soft_memory_under_test', _MODULE_PATH)
_gprd_soft_memory = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _gprd_soft_memory
_SPEC.loader.exec_module(_gprd_soft_memory)

GPRDRelation = _gprd_soft_memory.GPRDRelation
build_gprd_teacher = _gprd_soft_memory.build_gprd_teacher
gprd_ranking_loss = _gprd_soft_memory.gprd_ranking_loss
...
def test_gprd_loss_is_finite_and_backpropagates_with_candidates():
    ...
    loss, stats = gprd_ranking_loss(
        ...
        temperature=0.08, margin=0.04, pairwise_weight=0.10)
    loss.backward()
...
if __name__ == '__main__':
    ...
    print('GPRD-SOFT smoke test passed')
