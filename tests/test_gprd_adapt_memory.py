...
_MODULE_PATH = (Path(__file__).resolve().parents[1] /
                'clustercontrast' / 'models' / 'gprd_adapt_memory.py')
_SPEC = importlib.util.spec_from_file_location(
    'gprd_adapt_memory_under_test', _MODULE_PATH)
_gprd_adapt_memory = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _gprd_adapt_memory
_SPEC.loader.exec_module(_gprd_adapt_memory)

GPRDRelation = _gprd_adapt_memory.GPRDRelation
build_gprd_teacher = _gprd_adapt_memory.build_gprd_teacher
gprd_ranking_loss = _gprd_adapt_memory.gprd_ranking_loss


def test_gprd_adapt_teacher_keeps_stable_reciprocal_edges():
...
def test_gprd_adapt_loss_is_zero_without_active_candidates():
...
def test_gprd_adapt_loss_is_finite_and_backpropagates_with_candidates():
    relation = GPRDRelation(
...
def test_gprd_adapt_loss_filters_conflicts_and_reports_adaptive_scales():
    relation = GPRDRelation(
