"""Soft Graph-Preserved Reciprocal Distillation (GPRD-SOFT).
...
trainer then asks the raw/global embedding to match the whole teacher
neighbourhood distribution, with only a weak hard-negative term.
"""
...
def gprd_ranking_loss(student_features: torch.Tensor,
    ...
    soft_guard = (0.45 + 0.55 * raw_support).detach()
    ...
    weights = confidence * soft_guard * row_valid.to(confidence.dtype)
    per_row = (
        listwise
