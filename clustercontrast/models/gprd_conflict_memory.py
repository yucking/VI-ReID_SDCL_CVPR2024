"""Conflict-aware Graph-Preserved Reciprocal Distillation (GPRD-CONFLICT).
...
This variant keeps the GPRD soft neighbourhood teacher, but treats the
non-positive candidates as hard negatives only when the current raw embedding
does not strongly contradict that assignment.  It is designed for the SYSU log
pattern where the teacher is stable but several nominal negatives are closer
than the positive anchor in raw cosine space.
"""
...
def gprd_ranking_loss(student_features: torch.Tensor,
                      ...
                      conflict_floor: float = 0.45) -> Tuple[torch.Tensor, Dict[str, float]]:
