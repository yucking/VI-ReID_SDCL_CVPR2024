"""Adaptive Graph-Preserved Reciprocal Distillation (GPRD-ADAPT).
...
def gprd_ranking_loss(student_features: torch.Tensor,
                      ...
                      adaptive_floor: float = 0.35) -> Tuple[torch.Tensor, Dict[str, float]]:
    ...
    target = target * (
        ...
        + neg_scale * reliable_negative_mask.to(target.dtype)
    )
    ...
    pull_scale = 1.0 + 0.75 * gap_deficit * (
        0.50 + 0.50 * positive_agreement.detach()
    )
    weights = (
