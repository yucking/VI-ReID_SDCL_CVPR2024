"""Graph-Preserved Reciprocal Distillation (GPRD).
...
def _build_direction(source_fused: torch.Tensor,
    ...
    for row in range(query_ids.size):
        ...
        positive_dist = []
        support_scores = []
        for col, (target_id, distance, support) in enumerate(positives):
        ...
        positive_dist = np.asarray(positive_dist, dtype=np.float32).reshape(1, -1)
        positive_probs = _softmax_negative(positive_dist, teacher_temperature).reshape(-1)
        row_teacher[row, :len(positive_probs)] = positive_probs
        entropy = -(
            positive_probs * np.log(np.clip(positive_probs, 1e-12, None))
        ).sum()
        norm_entropy[row] = float(entropy / math.log(float(max(2, len(positive_probs)))))
        ...
        row_confidence[row] = (
            0.25 * best_score
            + 0.35 * (1.0 - norm_entropy[row])
            + 0.20 * np.clip(margin[row] / 0.15, 0.0, 1.0)
            + 0.20 * support_score
        )
        row_valid[row] = (
...
def build_gprd_teacher(rgb_features: torch.Tensor,
    ...
    rgb_to_ir, rgb_stats = _build_direction(
        ...
        confidence_floor, entropy_ceiling, stability_rounds, min_stability, seed)
...
def gprd_ranking_loss(student_features: torch.Tensor,
    ...
    """Guard stable reciprocal distillation by the current raw neighbourhood."""
    ...
    pos_anchor = (pos_sim_each * target).sum(dim=1)
    ...
    pos_spread = (
        ((sim - pos_anchor.unsqueeze(1)).pow(2) * target).sum(dim=1)
    )

    weights = confidence * raw_support * row_valid.to(confidence.dtype)
    per_row = (
        0.50 * pull
        + float(pairwise_weight) * margin_loss
        + 0.20 * listwise
        + 0.03 * pos_spread
    )
