"""Pseudo-Label Guided Graph-Preserved Reciprocal Distillation (GPRD-PLG).
...
This variant keeps the conflict-aware guard, but adapts the loss pressure when
the current raw embedding strongly contradicts fixed hard-negative settings.
Reliable negatives still define the denominator, while the teacher target
places most mass on positives and positive pull strengthens when the raw gap is
still inverted.
"""
...
def _topk_smallest_masked(distance: np.ndarray,
                          mask: np.ndarray,
                          topk: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Top-k under a boolean mask, returning per-entry validity."""
    k = min(max(1, int(topk)), distance.shape[1])
    masked = np.where(mask, distance, np.inf).astype(np.float32, copy=False)
    ids = np.argsort(masked, axis=1)[:, :k].astype(np.int64)
    values = np.take_along_axis(masked, ids, axis=1)
    valid = np.isfinite(values)
    values = np.where(valid, values, 1.0).astype(np.float32, copy=False)
    return values, ids, valid


def _labels_to_numpy(labels: Optional[torch.Tensor],
                     total: int) -> Optional[np.ndarray]:
    if labels is None:
        return None
    labels_np = np.asarray(labels.detach().cpu().numpy(), dtype=np.int64).reshape(-1)
    if labels_np.shape[0] != int(total):
        raise ValueError('label array length does not match feature count.')
    return labels_np
...
def _build_direction(source_fused: torch.Tensor,
    ...
    source_label_np = _labels_to_numpy(source_labels, source_count)
    target_label_np = _labels_to_numpy(target_labels, target_count)
    use_label_guard = source_label_np is not None and target_label_np is not None
    ...
    label_positive_hits = 0.0
    label_positive_total = 0.0
    camera_entropies = []
    ...
    for round_id in range(stability_rounds):
        ...
        if use_label_guard:
            source_round_labels = source_label_np[query_ids]
            target_round_labels = target_label_np[gallery_ids]
            valid_source_label = source_round_labels >= 0
            valid_target_label = target_round_labels >= 0
            same_label = (
                valid_source_label[:, None]
                & valid_target_label[None, :]
                & (source_round_labels[:, None] == target_round_labels[None, :])
            )
            different_label = (
                valid_source_label[:, None]
                & valid_target_label[None, :]
            ...
            pos_dist, pos_pos, pos_valid = _topk_smallest_masked(
                final_distance, same_label, positive_count)
            neg_dist, neg_pos, neg_valid = _topk_smallest_masked(
                final_distance, different_label, hard_negative_count)
            selected_pos = np.concatenate((pos_pos, neg_pos), axis=1)
            selected_dist = np.concatenate((pos_dist, neg_dist), axis=1)
            candidate_valid = np.concatenate((pos_valid, neg_valid), axis=1)
            ...
            selected_target_ids = np.where(candidate_valid, selected_target_ids, -1)
            selected_dist = np.where(candidate_valid, selected_dist, 1.0)
            selected_agree = np.where(candidate_valid, selected_agree, 0.0)
            mutual_mask = pos_valid.copy()
            if int(mutual_topk) > 0:
                mutual_mask = mutual_mask & _mutual_positive_mask(
                    final_distance, pos_pos, positive_count, mutual_topk)
            label_positive_hits += float(mutual_mask.sum())
            label_positive_total += float(mutual_mask.size)
        else:
    ...
    metrics = {
        ...
        'raw_agreement_min': float(raw_agreement_min),
        'label_positive_rate': (
            float(label_positive_hits) / float(max(1.0, label_positive_total))
            if use_label_guard else 0.0),
    }
...
def build_gprd_teacher(rgb_features: torch.Tensor,
                       ...
                       rgb_labels: Optional[torch.Tensor] = None,
                       ir_labels: Optional[torch.Tensor] = None,
    ...
    rgb_to_ir, rgb_stats = _build_direction(
        rgb_fused, ir_fused, rgb_features, ir_features,
        rgb_labels, ir_labels, rgb_cameras, ir_cameras,
    ...
    ir_to_rgb, ir_stats = _build_direction(
        ir_fused, rgb_fused, ir_features, rgb_features,
        ir_labels, rgb_labels, ir_cameras, rgb_cameras,
        query_per_camera, gallery_per_camera, positive_count,
        hard_negative_count, mutual_topk, rerank_k1, rerank_k2,
        rerank_lambda, csls_neighbors, csls_blend, teacher_temperature,
        confidence_floor, entropy_ceiling, stability_rounds, min_stability,
    ...
    metrics = {
        ...
        'label_positive_rate': 0.5 * (
            rgb_stats['label_positive_rate'] + ir_stats['label_positive_rate']),
    }
    return rgb_to_ir, ir_to_rgb, metrics


def _cache_stats(relation: GPRDRelation,
                 carried_mask: Optional[torch.Tensor] = None) -> Dict[str, float]:
    valid = relation.valid
    active_rows = float(valid.sum().item())
    if carried_mask is None:
        carried = torch.zeros_like(relation.candidate_ids, dtype=torch.bool)
    else:
        carried = carried_mask & relation.candidate_ids.ge(0)
    if bool(valid.any().item()):
        confidence = float(relation.confidence[valid].mean().item())
    else:
        confidence = 0.0
    return {
        'cache_rows': active_rows,
        'cache_candidates': float(relation.candidate_ids.ge(0).sum().item()),
        'cache_carried_candidates': float(carried.sum().item()),
        'cache_mean_confidence': confidence,
        'cache_candidate_width': float(relation.candidate_ids.size(1)),
    }


@torch.no_grad()
def merge_gprd_teacher_cache(current: GPRDRelation,
                             cached: Optional[GPRDRelation],
                             carry_count: int = 8,
                             carry_weight: float = 0.50,
                             min_confidence: float = 0.20
                             ) -> Tuple[GPRDRelation, Dict[str, float]]:
    """Merge train-only cached teacher edges into this epoch's relation.

    The merge never creates a target from test data; it only replays prior
    train-split reciprocal edges for the same train source row. Current epoch
    candidates keep priority, while cached candidates are appended when they are
    not already present in the current row.
    """
    if cached is None or int(carry_count) <= 0:
        return current, _cache_stats(current)

    device = current.candidate_ids.device
    cached_ids = cached.candidate_ids.to(device)
    cached_probs = cached.teacher_probs.to(device)
    cached_pos = cached.positive_mask.to(device)
    cached_neg = cached.negative_mask.to(device)
    cached_agree = cached.raw_agreement.to(device)
    cached_conf = cached.confidence.to(device)
    cached_valid = cached.valid.to(device)

    row_count = current.candidate_ids.size(0)
    if cached_ids.size(0) != row_count:
        raise ValueError('cached relation row count does not match current relation.')

    current_ids = current.candidate_ids
    current_valid = current_ids.ge(0)
    cached_valid_candidates = cached_ids.ge(0) & cached_valid.view(-1, 1)
    cached_valid_candidates = cached_valid_candidates & (
        cached_conf.view(-1, 1) >= float(min_confidence))
    duplicate = (
        cached_ids.unsqueeze(1).eq(current_ids.clamp_min(-1).unsqueeze(2))
        & current_valid.unsqueeze(2)
    ).any(dim=1)
    keep_cached = cached_valid_candidates & ~duplicate
    if not bool(keep_cached.any().item()):
        return current, _cache_stats(current)

    carry_count = min(max(1, int(carry_count)), cached_ids.size(1))
    carry_weight = min(1.0, max(0.0, float(carry_weight)))
    score = cached_probs * cached_agree.clamp_min(0.0)
    score = score * (0.75 + 0.25 * cached_conf.view(-1, 1))
    score = score + cached_pos.to(score.dtype) * 10.0
    score = score.masked_fill(~keep_cached, -10000.0)
    top_values, top_indices = score.topk(carry_count, dim=1)
    carry_valid = top_values.gt(-9999.0)

    carry_ids = cached_ids.gather(1, top_indices).masked_fill(~carry_valid, -1)
    carry_probs = cached_probs.gather(1, top_indices) * carry_weight
    carry_probs = carry_probs.masked_fill(~carry_valid, 0.0)
    carry_pos = cached_pos.gather(1, top_indices) & carry_valid
    carry_neg = cached_neg.gather(1, top_indices) & carry_valid
    carry_agree = cached_agree.gather(1, top_indices).masked_fill(~carry_valid, 0.0)

    merged_ids = torch.cat((current.candidate_ids, carry_ids), dim=1)
    merged_probs = torch.cat((current.teacher_probs, carry_probs), dim=1)
    merged_pos = torch.cat((current.positive_mask, carry_pos), dim=1)
    merged_neg = torch.cat((current.negative_mask, carry_neg), dim=1)
    merged_agree = torch.cat((current.raw_agreement, carry_agree), dim=1)
    merged_valid_candidates = merged_ids.ge(0)
    merged_probs = merged_probs * merged_valid_candidates.to(merged_probs.dtype)
    merged_probs = merged_probs / merged_probs.sum(dim=1, keepdim=True).clamp_min(1e-12)

    carried_mask = torch.zeros_like(merged_ids, dtype=torch.bool)
    carried_mask[:, current.candidate_ids.size(1):] = carry_valid
    replay_rows = carry_valid.any(dim=1)
    merged_confidence = torch.maximum(
        current.confidence,
        cached_conf * replay_rows.to(cached_conf.dtype) * carry_weight)
    merged_valid = (
        current.valid |
        (replay_rows & merged_pos.any(dim=1) & merged_neg.any(dim=1))
    )
    merged = GPRDRelation(
        candidate_ids=merged_ids.detach(),
        teacher_probs=merged_probs.detach(),
        positive_mask=merged_pos.detach(),
        negative_mask=merged_neg.detach(),
        raw_agreement=merged_agree.detach(),
        confidence=merged_confidence.detach(),
        valid=merged_valid.detach(),
        positive_count=current.positive_count,
        hard_negative_count=current.hard_negative_count,
    )
    return merged, _cache_stats(merged, carried_mask)


def _aggregate_hard_negative(sim: torch.Tensor,
                             negative_mask: torch.Tensor,
                             topk: int) -> torch.Tensor:
    """Return a top-k mean hard negative without logsumexp count inflation."""
    safe_sim = sim.masked_fill(~negative_mask, -10000.0)
    max_k = max(1, min(int(topk), safe_sim.size(1)))
    top_values = safe_sim.topk(max_k, dim=1).values
    counts = negative_mask.sum(dim=1).clamp_min(1)
    valid_top = (
        torch.arange(max_k, device=sim.device).view(1, -1)
        < counts.view(-1, 1)
    )
    top_values = top_values.masked_fill(~valid_top, 0.0)
    denom = valid_top.sum(dim=1).clamp_min(1).to(sim.dtype)
...
def gprd_ranking_loss(student_features: torch.Tensor,
                      ...
                      agreement_floor: float = 0.35,
                      conflict_margin: float = 0.02,
                      conflict_floor: float = 0.45,
                      negative_teacher_scale: float = 0.35,
                      pull_weight: float = 0.16,
                      spread_weight: float = 0.02,
                      adaptive_floor: float = 0.35,
                      focus_weight: float = 1.35,
                      negative_ceiling: float = 0.50,
                      negative_ceiling_weight: float = 0.08,
                      hard_negative_topk: int = 1,
                      listwise_weight: float = 1.0,
                      loss_confidence_floor: float = 0.0,
                      gap_floor: float = 0.06,
                      gap_weight: float = 0.0,
                      positive_floor: float = 0.48,
                      positive_floor_weight: float = 0.0,
                      structure_weight: float = 0.0) -> Tuple[torch.Tensor, Dict[str, float]]:
    """Distill reciprocal neighbours as a gap-focused ranking correction."""
    ...
    if relation is None or source_indices.numel() == 0:
        return zero, {
            ...
            'raw_agreement': 0.0,
            'conflict_rate': 0.0,
            'reliable_negatives': 0.0,
            'effective_pairwise': 0.0,
            'pull_scale': 0.0,
            'row_boost': 0.0,
            'neg_teacher_scale': 0.0,
            'negative_ceiling': 0.0,
            'hard_negative_topk': 0.0,
            'gap': 0.0,
            'gap_floor_loss': 0.0,
            'positive_floor': 0.0,
            'structure': 0.0,
    ...
    if float(loss_confidence_floor) > 0.0:
        row_valid = row_valid & confidence.ge(float(loss_confidence_floor))
    ...
    base_target = teacher_probs * candidate_valid.to(teacher_probs.dtype)
    base_target = base_target / base_target.sum(dim=1, keepdim=True).clamp_min(1e-12)
    base_positive_target = base_target * positive_mask.to(base_target.dtype)
    base_positive_target = (
        base_positive_target /
        base_positive_target.sum(dim=1, keepdim=True).clamp_min(1e-12)
    )
    base_pos_anchor = (
        sim.masked_fill(~positive_mask, 0.0) * base_positive_target
    ).sum(dim=1)

    conflict_negative = (
        negative_mask
        & (sim.detach() > (
            base_pos_anchor.detach().unsqueeze(1) + float(conflict_margin)))
    )
    reliable_negative_mask = negative_mask & ~conflict_negative
    fallback_ids = sim.masked_fill(~negative_mask, 10000.0).argmin(dim=1)
    fallback_negative = torch.zeros_like(negative_mask)
    fallback_negative.scatter_(1, fallback_ids.view(-1, 1), True)
    fallback_negative = fallback_negative & negative_mask
    reliable_negative_mask = reliable_negative_mask | (
        fallback_negative & ~reliable_negative_mask.any(dim=1, keepdim=True)
    )
    loss_candidate_valid = candidate_valid & (positive_mask | reliable_negative_mask)
    row_valid = valid & positive_mask.any(dim=1) & reliable_negative_mask.any(dim=1)
    if float(loss_confidence_floor) > 0.0:
        row_valid = row_valid & confidence.ge(float(loss_confidence_floor))
    if not bool(row_valid.any().item()):
        return zero, {
    ...
    hard_negative_topk = max(1, int(hard_negative_topk))
    prelim_hard_neg = _aggregate_hard_negative(
        sim.detach(), reliable_negative_mask, hard_negative_topk)
    prelim_gap = base_pos_anchor - prelim_hard_neg
    focus_tau = max(tau, 0.04)
    gap_focus = torch.sigmoid((float(margin) - prelim_gap) / focus_tau).detach()
    teacher_quality = (0.50 + 0.50 * positive_agreement.detach()).clamp(0.0, 1.0)
    focus_weight = max(0.0, float(focus_weight))
    row_boost = (1.0 + focus_weight * gap_focus * teacher_quality).clamp(
        max=1.0 + focus_weight)
    neg_teacher_gate = (1.0 - 0.85 * gap_focus * teacher_quality).clamp(
        min=0.05, max=1.0)

    target = teacher_probs * loss_candidate_valid.to(teacher_probs.dtype)
    neg_scale = min(1.0, max(0.0, float(negative_teacher_scale)))
    target = target * (
        positive_mask.to(target.dtype)
        + neg_scale * neg_teacher_gate.unsqueeze(1).to(target.dtype)
        * reliable_negative_mask.to(target.dtype)
    )
    target = target / target.sum(dim=1, keepdim=True).clamp_min(1e-12)
    logits = sim.masked_fill(~loss_candidate_valid, -10000.0) / tau
    log_probs = F.log_softmax(logits, dim=1)
    listwise = -(target * log_probs).sum(dim=1)
    local_count = loss_candidate_valid.sum(dim=1).clamp_min(1).to(sim.dtype)
    student_local = sim.masked_fill(~loss_candidate_valid, 0.0)
    teacher_local = (
        tau * target.clamp_min(1e-6).log()
    ).masked_fill(~loss_candidate_valid, 0.0)
    student_local = student_local - (
        student_local.sum(dim=1, keepdim=True) / local_count.view(-1, 1))
    teacher_local = teacher_local - (
        teacher_local.sum(dim=1, keepdim=True) / local_count.view(-1, 1))
    structure = F.smooth_l1_loss(
        student_local, teacher_local, reduction='none', beta=0.02)
    structure = (
        structure * loss_candidate_valid.to(structure.dtype)
    ).sum(dim=1) / local_count
    ...
    hard_neg = _aggregate_hard_negative(
        sim, reliable_negative_mask, hard_negative_topk)
    gap = pos_anchor - hard_neg
    gap_floor = max(0.0, float(gap_floor))
    top_gap_loss = F.softplus((gap_floor - gap) / tau) * tau
    all_neg_gap_loss = F.softplus(
        (sim - pos_anchor.unsqueeze(1) + gap_floor) / tau) * tau
    all_neg_gap_loss = (
        all_neg_gap_loss * reliable_negative_mask.to(all_neg_gap_loss.dtype)
    ).sum(dim=1) / reliable_negative_mask.sum(dim=1).clamp_min(1).to(sim.dtype)
    gap_floor_loss = 0.5 * top_gap_loss + 0.5 * all_neg_gap_loss
    positive_floor_loss = (
        F.softplus((float(positive_floor) - pos_anchor) / tau) * tau
    )
    ...
    neg_ceiling = float(negative_ceiling)
    negative_ceiling_loss = F.softplus((hard_neg - neg_ceiling) / tau) * tau
    pos_spread = (
        ((sim - pos_anchor.unsqueeze(1)).pow(2) * positive_target).sum(dim=1)
    )
    conflict_count = (conflict_negative & negative_mask).sum(dim=1).to(confidence.dtype)
    negative_count = negative_mask.sum(dim=1).clamp_min(1).to(confidence.dtype)
    conflict_ratio = (conflict_count / negative_count).detach()
    conflict_floor = min(1.0, max(0.0, float(conflict_floor)))
    conflict_guard = (
        conflict_floor + (1.0 - conflict_floor) * (1.0 - conflict_ratio)
    )
    reliable_count = reliable_negative_mask.sum(dim=1).to(confidence.dtype)
    reliable_ratio = (reliable_count / negative_count).detach()

    adaptive_floor = min(1.0, max(0.0, float(adaptive_floor)))
    gap_deficit = torch.sigmoid((-gap - 0.02) / 0.05).detach()
    reliability_guard = (
        adaptive_floor + (1.0 - adaptive_floor) * reliable_ratio
    )
    conflict_pairwise_guard = (
        1.0 - 0.45 * gap_deficit * conflict_ratio
    ).clamp(min=0.15)
    pairwise_scale = (
        (0.50 + 0.50 * positive_agreement.detach())
        * reliability_guard
        * conflict_pairwise_guard
        * row_boost
    )
    pull_scale = 1.0 + (0.75 + 0.35 * focus_weight) * gap_deficit * (
        0.50 + 0.50 * positive_agreement.detach()
    )
    weights = (
        confidence * soft_guard * agreement_guard * conflict_guard
        * row_valid.to(confidence.dtype)
    )
    per_row = (
        float(listwise_weight) * listwise
        + float(pairwise_weight) * pairwise_scale * margin_loss
        + max(0.0, float(gap_weight)) * row_boost * gap_floor_loss
        + float(pull_weight) * pull_scale * pull
        + max(0.0, float(positive_floor_weight)) * pull_scale * positive_floor_loss
        + max(0.0, float(structure_weight)) * structure
        + float(negative_ceiling_weight) * row_boost * negative_ceiling_loss
        + float(spread_weight) * pos_spread
    )
    ...
    return loss, {
        ...
        'conflict_rate': float(conflict_ratio[row_valid].mean().item()),
        'reliable_negatives': float(reliable_count[row_valid].mean().item()),
        'effective_pairwise': float(
            (float(pairwise_weight) * pairwise_scale)[row_valid].mean().item()),
        'pull_scale': float(pull_scale[row_valid].mean().item()),
        'row_boost': float(row_boost[row_valid].mean().item()),
        'neg_teacher_scale': float(
            (neg_scale * neg_teacher_gate)[row_valid].mean().item()),
        'negative_ceiling': float(negative_ceiling_loss[row_valid].mean().item()),
        'hard_negative_topk': float(hard_negative_topk),
        'gap': float(gap[row_valid].mean().item()),
        'gap_floor_loss': float(gap_floor_loss[row_valid].mean().item()),
        'positive_floor': float(positive_floor_loss[row_valid].mean().item()),
        'structure': float(structure[row_valid].mean().item()),
    }
