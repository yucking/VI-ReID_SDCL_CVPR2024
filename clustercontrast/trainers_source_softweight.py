...
class ClusterContrastTrainer_SDCL(object):
    def __init__(self, encoder, memory=None,matcher_rgb = None,matcher_ir = None):
        ...
        self.enable_proto_bridge = False
        self.proto_bridge_weight = 0.0
        self.proto_bridge_label_ir = None
        self.proto_bridge_label_rgb = None
        self.proto_bridge_memory = None
        self.proto_bridge_memory_s = None
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer, print_freq=10, train_iters=400):
        ...
        for i in range(train_iters):
            ...
            bridge_loss = torch.tensor([0.]).cuda()
            bridge_loss_s = torch.tensor([0.]).cuda()
            ...
            if (
                getattr(self, 'enable_proto_bridge', False)
                and epoch < self.cmlabel
                and getattr(self, 'proto_bridge_memory', None) is not None
                and getattr(self, 'proto_bridge_memory_s', None) is not None
                and getattr(self, 'proto_bridge_label_ir', None) is not None
                and getattr(self, 'proto_bridge_label_rgb', None) is not None
                and float(getattr(self, 'proto_bridge_weight', 0.0)) > 0.0
            ):
                bridge_labels_ir = self.proto_bridge_label_ir[index_ir].long().cuda()
                bridge_labels_rgb = self.proto_bridge_label_rgb[index_rgb].long().cuda()
                bridge_mask_ir = bridge_labels_ir >= 0
                bridge_mask_rgb = bridge_labels_rgb >= 0
                if bridge_mask_ir.any():
                    bridge_loss = bridge_loss + self.proto_bridge_memory(
                        f_out_ir[bridge_mask_ir], bridge_labels_ir[bridge_mask_ir])
                    bridge_loss_s = bridge_loss_s + self.proto_bridge_memory_s(
                        f_out_ir_s[bridge_mask_ir], bridge_labels_ir[bridge_mask_ir])
                if bridge_mask_rgb.any():
                    bridge_loss = bridge_loss + self.proto_bridge_memory(
                        f_out_rgb[bridge_mask_rgb], bridge_labels_rgb[bridge_mask_rgb])
                    bridge_loss_s = bridge_loss_s + self.proto_bridge_memory_s(
                        f_out_rgb_s[bridge_mask_rgb], bridge_labels_rgb[bridge_mask_rgb])
                bridge_loss = float(self.proto_bridge_weight) * bridge_loss
                bridge_loss_s = float(self.proto_bridge_weight) * bridge_loss_s
...
##########################V4 wise neighbor
            ...
            # loss=loss_rgb_s+loss_ir_s+loss_ir+loss_rgb+0.5*(ir_ir_loss+rgb_rgb_loss+ir_ir_loss_s+rgb_rgb_loss_s)+0.5*(rgb_ir_loss+ir_rgb_loss+rgb_ir_loss_s+ir_rgb_loss_s)#+0*(loss_ins_ir+loss_ins_rgb+loss_ins_ir_s+loss_ins_rgb_s)#+0.1*(loss_camera_ir+loss_camera_rgb)
                ...
                loss = loss + bridge_loss + bridge_loss_s
            ...
            bridge_loss_log.update((bridge_loss + bridge_loss_s).item())
            ...
            if (i + 1) % print_freq == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i + 1, len(data_loader_rgb),
                              ...
                              bridge_loss_log.val,bridge_loss_log.avg))
                ...
                print('bridge_loss_s,bridge_loss',bridge_loss_s.item(),bridge_loss.item())
