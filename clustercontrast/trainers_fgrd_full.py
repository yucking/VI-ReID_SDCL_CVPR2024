...
from clustercontrast.models.fgrd_full_memory import fgrd_full_ranking_loss
...
class ClusterContrastTrainer_FGRD_FULL(object):
    def __init__(self, encoder, memory=None,matcher_rgb = None,matcher_ir = None):
        super(ClusterContrastTrainer_FGRD_FULL, self).__init__()
        ...
        self.hm=0
        self.ht=0
        # FGRD is intentionally an auxiliary retrieval term.  These fields do
        # not alter pseudo labels, samplers, or any original SDCL memory label.
        self.fgrd_rgb_to_ir = None
        self.fgrd_ir_to_rgb = None
        self.fgrd_base_weight = 0.0
        self.fgrd_active_weight = 0.0
        self.fgrd_temperature = 0.05
        self.fgrd_margin = 0.15
        self.fgrd_pairwise_weight = 0.50
        self.fgrd_target_ratio = 0.15
        self.fgrd_enabled = False
        self.fgrd_positive_count = 0
        self.fgrd_negative_count = 0

    def set_fgrd_teacher(self, rgb_to_ir, ir_to_rgb, base_weight,
                         active_weight, temperature, margin,
                         pairwise_weight, target_ratio):
        self.fgrd_rgb_to_ir = rgb_to_ir
        self.fgrd_ir_to_rgb = ir_to_rgb
        self.fgrd_base_weight = float(base_weight)
        self.fgrd_active_weight = float(active_weight)
        self.fgrd_temperature = float(temperature)
        self.fgrd_margin = float(margin)
        self.fgrd_pairwise_weight = float(pairwise_weight)
        self.fgrd_target_ratio = float(target_ratio)
        self.fgrd_enabled = True
        self.fgrd_positive_count = int(rgb_to_ir.positive_count)
        self.fgrd_negative_count = int(rgb_to_ir.negative_count)

    def clear_fgrd_teacher(self):
        self.fgrd_rgb_to_ir = None
        self.fgrd_ir_to_rgb = None
        self.fgrd_active_weight = 0.0
        self.fgrd_enabled = False
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer, print_freq=10, train_iters=400):
        ...
        fgrd_loss_log = AverageMeter()
        fgrd_active_rows_log = AverageMeter()
        fgrd_listwise_log = AverageMeter()
        fgrd_pairwise_log = AverageMeter()
        fgrd_effective_weight_log = AverageMeter()
        fgrd_ratio_log = AverageMeter()
...
##########################V4 wise neighbor
            ...
            # FGRD is a detached full-rerank ranking target over the existing
            # opposite-modality instance memory. It never rewrites labels,
            # samplers, or any original SDCL memory identity.
            fgrd_loss = loss.sum() * 0.0
            fgrd_active_rows = 0.0
            fgrd_listwise = 0.0
            fgrd_pairwise = 0.0
            fgrd_effective_weight = 0.0
            fgrd_ratio = 0.0
            if self.fgrd_enabled and self.fgrd_active_weight > 0.0:
                fgrd_rgb_loss, fgrd_rgb_stats = fgrd_full_ranking_loss(
                    f_out_rgb, index_rgb, self.wise_memory_ir.features,
                    self.fgrd_rgb_to_ir, temperature=self.fgrd_temperature,
                    margin=self.fgrd_margin,
                    pairwise_weight=self.fgrd_pairwise_weight)
                fgrd_ir_loss, fgrd_ir_stats = fgrd_full_ranking_loss(
                    f_out_ir, index_ir, self.wise_memory_rgb.features,
                    self.fgrd_ir_to_rgb, temperature=self.fgrd_temperature,
                    margin=self.fgrd_margin,
                    pairwise_weight=self.fgrd_pairwise_weight)
                active_terms = []
                if fgrd_rgb_stats['active_rows'] > 0.0:
                    active_terms.append(fgrd_rgb_loss)
                if fgrd_ir_stats['active_rows'] > 0.0:
                    active_terms.append(fgrd_ir_loss)
                if active_terms:
                    fgrd_loss = sum(active_terms) / float(len(active_terms))
                    desired_weight = (
                        float(self.fgrd_target_ratio) * loss.detach()
                        / fgrd_loss.detach().clamp_min(1e-6)
                    )
                    fgrd_effective_weight = min(
                        float(self.fgrd_active_weight),
                        float(desired_weight.item()))
                    loss = loss + fgrd_effective_weight * fgrd_loss
                    fgrd_ratio = float(
                        (fgrd_effective_weight * fgrd_loss.detach()
                         / loss.detach().clamp_min(1e-6)).item())
                fgrd_active_rows = (fgrd_rgb_stats['active_rows'] +
                                    fgrd_ir_stats['active_rows'])
                fgrd_listwise = 0.5 * (
                    fgrd_rgb_stats['listwise'] + fgrd_ir_stats['listwise'])
                fgrd_pairwise = 0.5 * (
                    fgrd_rgb_stats['pairwise'] + fgrd_ir_stats['pairwise'])
            ...
            fgrd_loss_log.update(fgrd_loss.item())
            fgrd_active_rows_log.update(fgrd_active_rows)
            fgrd_listwise_log.update(fgrd_listwise)
            fgrd_pairwise_log.update(fgrd_pairwise)
            fgrd_effective_weight_log.update(fgrd_effective_weight)
            fgrd_ratio_log.update(fgrd_ratio)
            ...
            if (i + 1) % print_freq == 0:
                ...
                print('[FGRD-FULL] epoch={} fgrd_loss={:.6f} max_weight={:.6f} '
                      'effective_weight={:.6f} aux_ratio={:.4f} '
                      'active_rows={:.1f} positives={} far_negatives={} '
                      'listwise={:.4f} pairwise={:.4f}'.format(
                          epoch, fgrd_loss_log.avg, self.fgrd_active_weight,
                          fgrd_effective_weight_log.avg, fgrd_ratio_log.avg,
                          fgrd_active_rows_log.avg, self.fgrd_positive_count,
                          self.fgrd_negative_count, fgrd_listwise_log.avg,
                          fgrd_pairwise_log.avg))
