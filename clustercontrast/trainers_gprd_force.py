...
from clustercontrast.models.gprd_force_memory import gprd_ranking_loss
...
class ClusterContrastTrainer_GPRD(object):
    def __init__(self, encoder, memory=None,matcher_rgb = None,matcher_ir = None):
        super(ClusterContrastTrainer_GPRD, self).__init__()
        ...
        self.hm=0
        self.ht=0
        # GPRD is intentionally an auxiliary retrieval term.  These fields do
        # not alter pseudo labels, samplers, or any original SDCL memory label.
        self.GPRD_rgb_to_ir = None
        self.GPRD_ir_to_rgb = None
        self.GPRD_base_weight = 0.0
        self.GPRD_active_weight = 0.0
        self.GPRD_temperature = 0.05
        self.GPRD_margin = 0.15
        self.GPRD_pairwise_weight = 0.50
        self.GPRD_agreement_floor = 0.35
        self.GPRD_conflict_margin = 0.02
        self.GPRD_conflict_floor = 0.45
        self.GPRD_negative_teacher_scale = 0.35
        self.GPRD_pull_weight = 0.16
        self.GPRD_spread_weight = 0.02
        self.GPRD_adaptive_floor = 0.35
        self.GPRD_focus_weight = 1.35
        self.GPRD_negative_ceiling = 0.50
        self.GPRD_negative_ceiling_weight = 0.08
        self.GPRD_hard_negative_topk = 1
        self.GPRD_soft_branch_weight = 0.45
        self.GPRD_listwise_weight = 1.0
        self.GPRD_loss_confidence_floor = 0.0
        self.GPRD_FORCE_floor = 0.06
        self.GPRD_FORCE_weight = 0.0
        self.GPRD_positive_floor = 0.48
        self.GPRD_positive_floor_weight = 0.0
        self.GPRD_structure_weight = 0.0
        self.GPRD_enabled = False
        self.GPRD_positive_count = 0
        self.GPRD_hard_negative_count = 0

    def set_gprd_teacher(self, rgb_to_ir, ir_to_rgb, base_weight,
                         active_weight, temperature, margin,
                         pairwise_weight, agreement_floor=0.35,
                         conflict_margin=0.02, conflict_floor=0.45,
                         negative_teacher_scale=0.35, pull_weight=0.16,
                         spread_weight=0.02, adaptive_floor=0.35,
                         focus_weight=1.35, negative_ceiling=0.50,
                         negative_ceiling_weight=0.08,
                         hard_negative_topk=1,
                         soft_branch_weight=0.45,
                         listwise_weight=1.0,
                         loss_confidence_floor=0.0,
                         gap_floor=0.06,
                         gap_weight=0.0,
                         positive_floor=0.48,
                         positive_floor_weight=0.0,
                         structure_weight=0.0):
        self.GPRD_rgb_to_ir = rgb_to_ir
        self.GPRD_ir_to_rgb = ir_to_rgb
        self.GPRD_base_weight = float(base_weight)
        self.GPRD_active_weight = float(active_weight)
        self.GPRD_temperature = float(temperature)
        self.GPRD_margin = float(margin)
        self.GPRD_pairwise_weight = float(pairwise_weight)
        self.GPRD_agreement_floor = float(agreement_floor)
        self.GPRD_conflict_margin = float(conflict_margin)
        self.GPRD_conflict_floor = float(conflict_floor)
        self.GPRD_negative_teacher_scale = float(negative_teacher_scale)
        self.GPRD_pull_weight = float(pull_weight)
        self.GPRD_spread_weight = float(spread_weight)
        self.GPRD_adaptive_floor = float(adaptive_floor)
        self.GPRD_focus_weight = float(focus_weight)
        self.GPRD_negative_ceiling = float(negative_ceiling)
        self.GPRD_negative_ceiling_weight = float(negative_ceiling_weight)
        self.GPRD_hard_negative_topk = int(hard_negative_topk)
        self.GPRD_soft_branch_weight = float(soft_branch_weight)
        self.GPRD_listwise_weight = float(listwise_weight)
        self.GPRD_loss_confidence_floor = float(loss_confidence_floor)
        self.GPRD_FORCE_floor = float(gap_floor)
        self.GPRD_FORCE_weight = float(gap_weight)
        self.GPRD_positive_floor = float(positive_floor)
        self.GPRD_positive_floor_weight = float(positive_floor_weight)
        self.GPRD_structure_weight = float(structure_weight)
        self.GPRD_enabled = True
        self.GPRD_positive_count = int(rgb_to_ir.positive_count)
        self.GPRD_hard_negative_count = int(rgb_to_ir.hard_negative_count)

    def clear_gprd_teacher(self):
        self.GPRD_rgb_to_ir = None
        self.GPRD_ir_to_rgb = None
        self.GPRD_active_weight = 0.0
        self.GPRD_enabled = False
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer, print_freq=10, train_iters=400):
        ...
        GPRD_loss_log = AverageMeter()
        GPRD_active_rows_log = AverageMeter()
        GPRD_listwise_log = AverageMeter()
        GPRD_pairwise_log = AverageMeter()
        GPRD_pull_log = AverageMeter()
        GPRD_margin_log = AverageMeter()
        GPRD_pos_sim_log = AverageMeter()
        GPRD_neg_sim_log = AverageMeter()
        GPRD_raw_support_log = AverageMeter()
        GPRD_raw_agreement_log = AverageMeter()
        GPRD_conflict_rate_log = AverageMeter()
        GPRD_reliable_negatives_log = AverageMeter()
        GPRD_effective_pairwise_log = AverageMeter()
        GPRD_pull_scale_log = AverageMeter()
        GPRD_row_boost_log = AverageMeter()
        GPRD_neg_teacher_scale_log = AverageMeter()
        GPRD_negative_ceiling_log = AverageMeter()
        GPRD_hard_negative_topk_log = AverageMeter()
        GPRD_FORCE_log = AverageMeter()
        GPRD_FORCE_floor_loss_log = AverageMeter()
        GPRD_positive_floor_log = AverageMeter()
        GPRD_structure_log = AverageMeter()
        GPRD_soft_rows_log = AverageMeter()
        GPRD_soft_gap_log = AverageMeter()
...
##########################V4 wise neighbor
            ...
            # GPRD is a detached stable reciprocal ranking target over the existing
            # opposite-modality instance memory. It never rewrites labels,
            # samplers, or any original SDCL memory identity.
            GPRD_loss = loss.sum() * 0.0
            GPRD_active_rows = 0.0
            GPRD_listwise = 0.0
            GPRD_pairwise = 0.0
            GPRD_pull = 0.0
            GPRD_margin = 0.0
            GPRD_pos_sim = 0.0
            GPRD_neg_sim = 0.0
            GPRD_raw_support = 0.0
            GPRD_raw_agreement = 0.0
            GPRD_conflict_rate = 0.0
            GPRD_reliable_negatives = 0.0
            GPRD_effective_pairwise = 0.0
            GPRD_pull_scale = 0.0
            GPRD_row_boost = 0.0
            GPRD_neg_teacher_scale = 0.0
            GPRD_negative_ceiling = 0.0
            GPRD_hard_negative_topk = 0.0
            GPRD_FORCE = 0.0
            GPRD_FORCE_floor_loss = 0.0
            GPRD_positive_floor = 0.0
            GPRD_structure = 0.0
            GPRD_soft_rows = 0.0
            GPRD_soft_gap = 0.0
            if self.GPRD_enabled and self.GPRD_active_weight > 0.0:
                GPRD_rgb_loss, GPRD_rgb_stats = gprd_ranking_loss(
                    f_out_rgb, index_rgb, self.wise_memory_ir.features,
                    self.GPRD_rgb_to_ir, temperature=self.GPRD_temperature,
                    margin=self.GPRD_margin,
                    pairwise_weight=self.GPRD_pairwise_weight,
                    agreement_floor=self.GPRD_agreement_floor,
                    conflict_margin=self.GPRD_conflict_margin,
                    conflict_floor=self.GPRD_conflict_floor,
                    negative_teacher_scale=self.GPRD_negative_teacher_scale,
                    pull_weight=self.GPRD_pull_weight,
                    spread_weight=self.GPRD_spread_weight,
                    adaptive_floor=self.GPRD_adaptive_floor,
                    focus_weight=self.GPRD_focus_weight,
                    negative_ceiling=self.GPRD_negative_ceiling,
                    negative_ceiling_weight=self.GPRD_negative_ceiling_weight,
                    hard_negative_topk=self.GPRD_hard_negative_topk,
                    listwise_weight=self.GPRD_listwise_weight,
                    loss_confidence_floor=self.GPRD_loss_confidence_floor,
                    gap_floor=self.GPRD_FORCE_floor,
                    gap_weight=self.GPRD_FORCE_weight,
                    positive_floor=self.GPRD_positive_floor,
                    positive_floor_weight=self.GPRD_positive_floor_weight,
                    structure_weight=self.GPRD_structure_weight)
                GPRD_ir_loss, GPRD_ir_stats = gprd_ranking_loss(
                    f_out_ir, index_ir, self.wise_memory_rgb.features,
                    self.GPRD_ir_to_rgb, temperature=self.GPRD_temperature,
                    margin=self.GPRD_margin,
                    pairwise_weight=self.GPRD_pairwise_weight,
                    agreement_floor=self.GPRD_agreement_floor,
                    conflict_margin=self.GPRD_conflict_margin,
                ...
                active_stats = []
                soft_stats = []
                if GPRD_rgb_stats['active_rows'] > 0.0:
                    active_terms.append((GPRD_rgb_loss, 1.0))
                    active_stats.append(GPRD_rgb_stats)
                if GPRD_ir_stats['active_rows'] > 0.0:
                    active_terms.append((GPRD_ir_loss, 1.0))
                    active_stats.append(GPRD_ir_stats)
                if self.GPRD_soft_branch_weight > 0.0:
                    GPRD_rgb_s_loss, GPRD_rgb_s_stats = gprd_ranking_loss(
                        f_out_rgb_s, index_rgb, self.wise_memory_ir_s.features,
                        self.GPRD_rgb_to_ir, temperature=self.GPRD_temperature,
                        margin=self.GPRD_margin,
                        pairwise_weight=self.GPRD_pairwise_weight,
                        agreement_floor=self.GPRD_agreement_floor,
                        ...
                        positive_floor_weight=self.GPRD_positive_floor_weight,
                        structure_weight=self.GPRD_structure_weight)
                    GPRD_ir_s_loss, GPRD_ir_s_stats = gprd_ranking_loss(
                        f_out_ir_s, index_ir, self.wise_memory_rgb_s.features,
                        self.GPRD_ir_to_rgb, temperature=self.GPRD_temperature,
                        margin=self.GPRD_margin,
                        pairwise_weight=self.GPRD_pairwise_weight,
                        agreement_floor=self.GPRD_agreement_floor,
                        ...
                        positive_floor_weight=self.GPRD_positive_floor_weight,
                        structure_weight=self.GPRD_structure_weight)
                    if GPRD_rgb_s_stats['active_rows'] > 0.0:
                        active_terms.append((GPRD_rgb_s_loss, self.GPRD_soft_branch_weight))
                        active_stats.append(GPRD_rgb_s_stats)
                        soft_stats.append(GPRD_rgb_s_stats)
                    if GPRD_ir_s_stats['active_rows'] > 0.0:
                        active_terms.append((GPRD_ir_s_loss, self.GPRD_soft_branch_weight))
                        active_stats.append(GPRD_ir_s_stats)
                        soft_stats.append(GPRD_ir_s_stats)
                if active_terms:
                    total_weight = sum(weight for _, weight in active_terms)
                    GPRD_loss = sum(loss_term * weight for loss_term, weight in active_terms)
                    GPRD_loss = GPRD_loss / float(max(total_weight, 1e-12))
                    loss = loss + self.GPRD_active_weight * GPRD_loss
                GPRD_active_rows = sum(stats['active_rows'] for stats in active_stats)
                if soft_stats:
                    soft_denom = float(len(soft_stats))
                    GPRD_soft_rows = sum(stats['active_rows'] for stats in soft_stats)
                    GPRD_soft_gap = sum(stats['gap'] for stats in soft_stats) / soft_denom
                if active_stats:
                    denom = float(len(active_stats))
                    GPRD_listwise = sum(stats['listwise'] for stats in active_stats) / denom
                    GPRD_pairwise = sum(stats['pairwise'] for stats in active_stats) / denom
                    GPRD_pull = sum(stats['pull'] for stats in active_stats) / denom
                    GPRD_margin = sum(stats['margin'] for stats in active_stats) / denom
                    GPRD_pos_sim = sum(stats['pos_sim'] for stats in active_stats) / denom
                    GPRD_neg_sim = sum(stats['neg_sim'] for stats in active_stats) / denom
                    GPRD_raw_support = sum(stats['raw_support'] for stats in active_stats) / denom
                    GPRD_raw_agreement = sum(stats['raw_agreement'] for stats in active_stats) / denom
                    GPRD_conflict_rate = sum(stats['conflict_rate'] for stats in active_stats) / denom
                    GPRD_reliable_negatives = sum(stats['reliable_negatives'] for stats in active_stats) / denom
                    GPRD_effective_pairwise = sum(stats['effective_pairwise'] for stats in active_stats) / denom
                    GPRD_pull_scale = sum(stats['pull_scale'] for stats in active_stats) / denom
                    GPRD_row_boost = sum(stats['row_boost'] for stats in active_stats) / denom
                    GPRD_neg_teacher_scale = sum(stats['neg_teacher_scale'] for stats in active_stats) / denom
                    GPRD_negative_ceiling = sum(stats['negative_ceiling'] for stats in active_stats) / denom
                    GPRD_hard_negative_topk = sum(stats['hard_negative_topk'] for stats in active_stats) / denom
                    GPRD_FORCE = sum(stats['gap'] for stats in active_stats) / denom
                    GPRD_FORCE_floor_loss = sum(stats['gap_floor_loss'] for stats in active_stats) / denom
                    GPRD_positive_floor = sum(stats['positive_floor'] for stats in active_stats) / denom
                    GPRD_structure = sum(stats['structure'] for stats in active_stats) / denom
            ...
            GPRD_loss_log.update(GPRD_loss.item())
            GPRD_active_rows_log.update(GPRD_active_rows)
            GPRD_listwise_log.update(GPRD_listwise)
            GPRD_pairwise_log.update(GPRD_pairwise)
            GPRD_pull_log.update(GPRD_pull)
            GPRD_margin_log.update(GPRD_margin)
            GPRD_pos_sim_log.update(GPRD_pos_sim)
            GPRD_neg_sim_log.update(GPRD_neg_sim)
            GPRD_raw_support_log.update(GPRD_raw_support)
            GPRD_raw_agreement_log.update(GPRD_raw_agreement)
            GPRD_conflict_rate_log.update(GPRD_conflict_rate)
            GPRD_reliable_negatives_log.update(GPRD_reliable_negatives)
            GPRD_effective_pairwise_log.update(GPRD_effective_pairwise)
            GPRD_pull_scale_log.update(GPRD_pull_scale)
            GPRD_row_boost_log.update(GPRD_row_boost)
            GPRD_neg_teacher_scale_log.update(GPRD_neg_teacher_scale)
            GPRD_negative_ceiling_log.update(GPRD_negative_ceiling)
            GPRD_hard_negative_topk_log.update(GPRD_hard_negative_topk)
            GPRD_FORCE_log.update(GPRD_FORCE)
            GPRD_FORCE_floor_loss_log.update(GPRD_FORCE_floor_loss)
            GPRD_positive_floor_log.update(GPRD_positive_floor)
            GPRD_structure_log.update(GPRD_structure)
            GPRD_soft_rows_log.update(GPRD_soft_rows)
            GPRD_soft_gap_log.update(GPRD_soft_gap)
            ...
            if (i + 1) % print_freq == 0:
                ...
                print('[GPRD] epoch={} gprd_loss={:.6f} active_weight={:.6f} '
                      'active_rows={:.1f} positives={} hard_negatives={} '
                      'pull={:.4f} margin={:.4f} listwise={:.4f} '
                      'pos_sim={:.4f} neg_sim={:.4f} raw_support={:.4f} '
                      'raw_agreement={:.4f} conflict_rate={:.4f} '
                      'reliable_negatives={:.2f} eff_pairwise={:.4f} '
                      'pull_scale={:.4f} row_boost={:.4f} '
                      'neg_teacher_scale={:.4f} neg_ceiling={:.4f} '
                      'hard_topk={:.1f} gap={:.4f} '
                      'gap_floor_loss={:.4f} pos_floor={:.4f} structure={:.4f} '
                      'soft_w={:.3f} soft_rows={:.1f} soft_gap={:.4f} '
                      'listwise_w={:.3f} gap_floor={:.3f} gap_w={:.3f} '
                      'conf_floor={:.3f}'.format(
                          epoch, GPRD_loss_log.avg, self.GPRD_active_weight,
                          GPRD_active_rows_log.avg, self.GPRD_positive_count,
                          self.GPRD_hard_negative_count, GPRD_pull_log.avg,
                          GPRD_margin_log.avg, GPRD_listwise_log.avg,
                          GPRD_pos_sim_log.avg, GPRD_neg_sim_log.avg,
                          GPRD_raw_support_log.avg,
                          GPRD_raw_agreement_log.avg,
                          GPRD_conflict_rate_log.avg,
                          GPRD_reliable_negatives_log.avg,
                          GPRD_effective_pairwise_log.avg,
                          GPRD_pull_scale_log.avg,
                          GPRD_row_boost_log.avg,
                          GPRD_neg_teacher_scale_log.avg,
                          GPRD_negative_ceiling_log.avg,
                          GPRD_hard_negative_topk_log.avg,
                          GPRD_FORCE_log.avg,
                          GPRD_FORCE_floor_loss_log.avg,
                          GPRD_positive_floor_log.avg,
                          GPRD_structure_log.avg,
                          self.GPRD_soft_branch_weight,
                          GPRD_soft_rows_log.avg,
                          GPRD_soft_gap_log.avg,
                          self.GPRD_listwise_weight,
                          self.GPRD_FORCE_floor,
                          self.GPRD_FORCE_weight,
                          self.GPRD_loss_confidence_floor))
