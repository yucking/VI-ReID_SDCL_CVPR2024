...
from clustercontrast.models.gprd_memory import gprd_ranking_loss
...
##########################V4 wise neighbor
            ...
            if self.GPRD_enabled and self.GPRD_active_weight > 0.0:
                ...
                if GPRD_rgb_stats['active_rows'] > 0.0:
                    active_terms.append(GPRD_rgb_loss)
                if GPRD_ir_stats['active_rows'] > 0.0:
                    active_terms.append(GPRD_ir_loss)
                if active_terms:
                    GPRD_loss = sum(active_terms) / float(len(active_terms))
                    loss = loss + self.GPRD_active_weight * GPRD_loss
                GPRD_active_rows = (GPRD_rgb_stats['active_rows'] +
                                    GPRD_ir_stats['active_rows'])
                GPRD_listwise = 0.5 * (
                    GPRD_rgb_stats['listwise'] + GPRD_ir_stats['listwise'])
                GPRD_pairwise = 0.5 * (
                    GPRD_rgb_stats['pairwise'] + GPRD_ir_stats['pairwise'])
            ...
            if (i + 1) % print_freq == 0:
                ...
                print('[GPRD] epoch={} gprd_loss={:.6f} active_weight={:.6f} '
                      ...
                      'listwise={:.4f} pairwise={:.4f}'.format(
                          ...
                          self.GPRD_hard_negative_count, GPRD_listwise_log.avg,
                          GPRD_pairwise_log.avg))
