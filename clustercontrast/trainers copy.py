...
class ClusterContrastTrainer_SDCL(object):
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer, print_freq=10, train_iters=400):
        ...
        for i in range(train_iters):
            ...
            cid_rgb, cid_ir, index_rgb, index_ir = self._forward(
                inputs_rgb, inputs_ir,
                label_1=labels_rgb, label_2=labels_ir, modal=0,
            ...
            # [DBG-L5] labels range check right before CM
            if epoch in [self.cmlabel, self.cmlabel + 1, self.cmlabel + 2] and (i % 100 == 0):
                with torch.no_grad():
                    # memory 的类数：优先用 features 行数
                    ncls = int(self.memory_ir.features.size(0)) if hasattr(self.memory_ir, "features") else -1

                    li = labels_ir.detach()
                    lr = labels_rgb.detach()

                    bad_ir  = (li.lt(0) | li.ge(ncls)).any().item() if ncls > 0 else False
                    bad_rgb = (lr.lt(0) | lr.ge(ncls)).any().item() if ncls > 0 else False

                    print(f"[DBG-L5] epoch={epoch} n_classes={ncls} "
                        f"| IR(min/max)={int(li.min())}/{int(li.max())} bad={bad_ir} "
                        f"| RGB(min/max)={int(lr.min())}/{int(lr.max())} bad={bad_rgb}")

                    if bad_ir:
                        bad_idx = torch.nonzero(li.lt(0) | li.ge(ncls), as_tuple=False)[:10].view(-1)
                        print("[DBG-L5] IR bad labels sample:", li[bad_idx].view(-1).tolist())

                    if bad_rgb:
                        bad_idx = torch.nonzero(lr.lt(0) | lr.ge(ncls), as_tuple=False)[:10].view(-1)
                        print("[DBG-L5] RGB bad labels sample:", lr[bad_idx].view(-1).tolist())

                    print(f"[DBG-L5] rgb_label_offset={getattr(self,'rgb_label_offset',None)}")
