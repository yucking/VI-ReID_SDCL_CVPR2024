...
class ClusterContrastTrainer_SDCL(object):
    def __init__(self, encoder, memory=None,matcher_rgb = None,matcher_ir = None):
        ...
        self.hm=0
        self.ht=0
        self.contrast_tau = 0.05

        # Optional reliability-aware training knobs (off by default).
        self.enable_reliability_weight = False
        self.rel_weight_power = 1.0
        self.sample_weight_ir = None
        self.sample_weight_rgb = None

        # Optional progressive neighborhood schedule (off by default).
        self.progressive_neighbor = False
        self.neighbor_thresh_base = 0.9
        self.hm_neighbor_thresh_base = 0.9
        self.neighbor_thresh_min = 0.82
        self.progressive_start = 0
        self.progressive_warmup = 10

        # Optional mixed precision for memory saving.
        self.use_amp = False
        self.scaler = None
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer, print_freq=10, train_iters=400):
        ...
        tau = max(float(getattr(self, 'contrast_tau', 0.05)), 1e-6)
        enable_rel = bool(getattr(self, 'enable_reliability_weight', False))
        rel_power = max(float(getattr(self, 'rel_weight_power', 1.0)), 0.0)
        grad_accum_steps = max(int(getattr(self, 'grad_accum_steps', 1)), 1)
        use_amp = bool(getattr(self, 'use_amp', False))
        if use_amp and getattr(self, 'scaler', None) is None:
            self.scaler = torch.cuda.amp.GradScaler(enabled=True)

        thresh = float(getattr(self, 'neighbor_thresh_base', 0.9))
        hm_thresh = float(getattr(self, 'hm_neighbor_thresh_base', 0.9))
        if bool(getattr(self, 'progressive_neighbor', False)):
            start = int(getattr(self, 'progressive_start', self.cmlabel))
            warmup = max(int(getattr(self, 'progressive_warmup', 10)), 1)
            progress = max(0.0, min(1.0, (epoch - start + 1) / float(warmup)))
            min_t = float(getattr(self, 'neighbor_thresh_min', 0.82))
            thresh = thresh - (thresh - min_t) * progress
            hm_thresh = hm_thresh - (hm_thresh - min_t) * progress

        def weighted_reduce(loss_vec, weights):
            if weights is None:
                return loss_vec.mean()
            if loss_vec.dim() > 1:
                loss_vec = loss_vec.view(-1)
            w = weights.view(-1).float().clamp_min(1e-6)
            if rel_power != 1.0:
                w = w.pow(rel_power)
            if w.numel() != loss_vec.numel():
                m = min(w.numel(), loss_vec.numel())
                w = w[:m]
                loss_vec = loss_vec[:m]
        ...
        for i in range(train_iters):
            ...
            if use_amp:
                f_out_rgb = f_out_rgb.float()
                f_out_ir = f_out_ir.float()
                f_out_rgb_s = f_out_rgb_s.float()
                f_out_ir_s = f_out_ir_s.float()
            ...
            ca_rgb = None
            if enable_rel:
                if getattr(self, 'sample_weight_ir', None) is not None:
                    ca_ir = self.sample_weight_ir[index_ir].detach()
                if getattr(self, 'sample_weight_rgb', None) is not None:
                    ca_rgb = self.sample_weight_rgb[index_rgb].detach()

            loss_ir_s = lamda_sd* self.memory_ir_s(f_out_ir_s, labels_ir, ca=ca_ir) 
            loss_rgb_s = lamda_sd* self.memory_rgb_s(f_out_rgb_s, labels_rgb, ca=ca_rgb)
            loss_ir = self.memory_ir(f_out_ir, labels_ir, ca=ca_ir)
            loss_rgb = self.memory_rgb(f_out_rgb, labels_rgb, ca=ca_rgb)
################cpsrefine
# #############v2
...
##########################V4 wise neighbor
                    if epoch %2 ==0:
                        ...
                        sim_rgb_ir_exp =sim_rgb_ir /tau  # 64*13638
                        ...
                        rgb_ir_loss = 0.1*lamda_d_neibor*weighted_reduce(rgb_ir_loss.div(num_neighbor_rgb_ir), ca_rgb)#.mul(rgb_ir_ca).mul(rgb_ir_ca).mul(rgb_ir_ca).mul(rgb_ir_ca).mul(mask_neighbor_intra_soft) ##.mul(rgb_ir_ca)
                        # print('rgb_ir_loss',rgb_ir_loss)
                        sim_prob_rgb_ir_exp =sim_prob_rgb_ir /tau  # 64*13638
                        ...
                        rgb_ir_loss_s = 0.1*lamda_s_neibor*weighted_reduce(rgb_ir_loss_s.div(num_neighbor_rgb_ir), ca_rgb)#.mul(rgb_ir_ca).mul(rgb_ir_ca) ##.mul(rgb_ir_ca)
                        ...
                        ir_rgb_loss = lamda_d_neibor*weighted_reduce(ir_rgb_loss.div(num_neighbor_rgb_ir), ca_ir)#.mul(ir_rgb_ca).mul(ir_rgb__ca)mul(rgb_ir_ca).mul(mask_neighbor_intra_soft) ##.mul(rgb_ir_ca)
                        ...
                        ir_rgb_loss_s = lamda_s_neibor*weighted_reduce(ir_rgb_loss_s.div(num_neighbor_rgb_ir), ca_ir)#.mul(ir_rgb_ca).mul(rgb_ir_ca) ##
                ...
                sim_rgb_rgb_exp =sim_rgb_rgb /tau  # 64*13638
                ...
                rgb_rgb_loss = lamda_d_neibor*weighted_reduce(rgb_rgb_loss.div(num_neighbor_rgb_rgb), ca_rgb)#.mul(rgb_ca).mul(rgb_ca).mul(rgb_ca)..mul(rgb_ca)mul(mask_neighbor_intra_soft) ##
                
                sim_prob_rgb_rgb_exp =sim_prob_rgb_rgb /tau  # 64*13638
                ...
                rgb_rgb_loss_s = lamda_s_neibor*weighted_reduce(rgb_rgb_loss_s.div(num_neighbor_rgb_rgb), ca_rgb)#.mul(rgb_ca).mul(rgb_ca).mul(mask_neighbor_intra_soft) ##
                ...
                sim_ir_ir_exp =sim_ir_ir /tau  # 64*13638
                ...
                ir_ir_loss = lamda_d_neibor*weighted_reduce(ir_ir_loss.div(num_neighbor_ir_ir), ca_ir)#.mul(ir_ca).mul(ir_ca).mul(ir_ca).mul(ir_ca).mul(mask_neighbor_intra_soft) ##


                sim_prob_ir_ir_exp =sim_prob_ir_ir /tau  # 64*13638
                ...
                ir_ir_loss_s = lamda_s_neibor*weighted_reduce(ir_ir_loss_s.div(num_neighbor_ir_ir), ca_ir)#.mul(ir_ca).mul(ir_ca).mul(mask_neighbor_intra_soft) ##
            ...
            if use_amp:
                ...
                if ((i + 1) % grad_accum_steps == 0) or (i + 1 == train_iters):
