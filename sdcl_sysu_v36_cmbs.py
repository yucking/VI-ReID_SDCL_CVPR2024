...
"""
SDCL v36 CMBS: cross-modal identity-balanced Stage-2.

v35 proved that full-state Stage-2 handoff changes metrics only slightly, so v36
moves the main closure instead of adding another neighbourhood teacher. During
Stage-2, CMBS constructs synchronized IR/RGB batches from shared pseudo labels:
each mini-batch contains the same identity blocks in both modalities. The source
trainer can then apply an explicit batch-level IR<->RGB multi-positive loss.
...
def main_worker_stage2(args,log_name):
    ...
    trainer.enable_cmbs_pair_loss = False
    trainer.cmbs_pair_weight = 0.0
    trainer.cmbs_pair_tau = 0.07
    ...
    def build_full_state_payload(epoch, next_epoch, source_stage1_checkpoint=''):
        return {
            ...
            'resume_kind': 'v36_full_state_stage2_handoff',
    ...
    def build_label_index(dataset):
        ...
        for item in dataset:
            _, pid, _ = item
            if int(pid) >= 0:
                label_index[int(pid)].append(item)
        return label_index

    def sample_camera_aware_block(items, num_instances):
        by_cam = collections.defaultdict(list)
        for item in items:
            by_cam[int(item[2])].append(item)

        cams = list(by_cam.keys())
        random.shuffle(cams)
        selected = []
        while len(selected) < num_instances and cams:
            for cam in cams:
                if len(selected) >= num_instances:
                    break
                selected.append(random.choice(by_cam[cam]))
            if len(selected) >= len(items) and len(selected) < num_instances:
                break
    ...
    def build_cmbs_trainsets(ir_set, rgb_set, batch_size, num_instances, train_iters):
        num_instances = max(1, int(num_instances))
        ids_per_batch = max(1, int(batch_size) // num_instances)
        effective_batch = ids_per_batch * num_instances
        ir_by_label = build_label_index(ir_set)
        rgb_by_label = build_label_index(rgb_set)
        common_labels = sorted(set(ir_by_label.keys()) & set(rgb_by_label.keys()))
        min_common = int(getattr(args, 'cmbs_min_common_ids', 64))
        stats = {
            'active': False,
            'common_ids': len(common_labels),
            'ids_per_batch': ids_per_batch,
            'instances': num_instances,
            'effective_batch': effective_batch,
            'target_iters': int(train_iters),
        }
        if len(common_labels) < min_common:
            stats['reason'] = 'common_ids_below_min'
            return ir_set, rgb_set, stats
        if effective_batch != int(batch_size):
            stats['reason'] = 'batch_size_not_divisible_by_instances'
            return ir_set, rgb_set, stats

        target_blocks = int(math.ceil(float(train_iters * effective_batch) / float(num_instances)))
        target_blocks = int(math.ceil(float(target_blocks) / float(ids_per_batch)) * ids_per_batch)
        label_order = []
        while len(label_order) < target_blocks:
            shuffled = list(common_labels)
            random.shuffle(shuffled)
            label_order.extend(shuffled)
        label_order = label_order[:target_blocks]

        balanced_ir = []
        balanced_rgb = []
        used_labels = set()
        for pid in label_order:
            balanced_ir.extend(sample_camera_aware_block(ir_by_label[pid], num_instances))
            balanced_rgb.extend(sample_camera_aware_block(rgb_by_label[pid], num_instances))
            used_labels.add(pid)

        stats.update({
            'active': True,
            'selected_blocks': len(label_order),
            'unique_used_ids': len(used_labels),
            'ir_samples': len(balanced_ir),
            'rgb_samples': len(balanced_rgb),
            'batches': len(balanced_ir) // max(1, effective_batch),
        })
        return balanced_ir, balanced_rgb, stats

    def get_ordered_train_loader_ir(dataset, trainset, train_transformer, length):
        return IterLoader(
            DataLoader(Preprocessor(trainset, root=dataset.images_dir, transform=train_transformer),
                       batch_size=args.batch_size, num_workers=args.workers,
                       shuffle=False, pin_memory=True, drop_last=True),
            length=length)

    def get_ordered_train_loader_rgb(dataset, trainset, train_transformer, train_transformer1, length):
        return IterLoader(
            DataLoader(Preprocessor_color(trainset, root=dataset.images_dir, transform=train_transformer,
...
############## Shared memory layer
        ...
        trainer.enable_cmbs_pair_loss = False
        trainer.cmbs_pair_weight = 0.0
        trainer.cmbs_pair_tau = float(getattr(args, 'cmbs_pair_tau', 0.07))
        ...
        cmbs_active = False
        cmbs_stats = None
        train_iters_target = int(iters) if iters is not None else int(args.iters)
        trainset_ir_for_loader = pseudo_labeled_dataset_ir
        trainset_rgb_for_loader = pseudo_labeled_dataset_rgb
        if bool(getattr(args, 'enable_cmbs', False)) and epoch >= trainer.cmlabel:
            trainset_ir_for_loader, trainset_rgb_for_loader, cmbs_stats = build_cmbs_trainsets(
                pseudo_labeled_dataset_ir,
                pseudo_labeled_dataset_rgb,
                int(args.batch_size),
                int(getattr(args, 'cmbs_num_instances', 8)),
                train_iters_target,
            )
            cmbs_active = bool(cmbs_stats.get('active', False))
            if cmbs_active:
                trainer.enable_cmbs_pair_loss = float(getattr(args, 'cmbs_pair_weight', 0.20)) > 0.0
                trainer.cmbs_pair_weight = float(getattr(args, 'cmbs_pair_weight', 0.20))
                trainer.cmbs_pair_tau = float(getattr(args, 'cmbs_pair_tau', 0.07))
                print('[CMBS] epoch={} active=1 common_ids={} unique_used={} ids_per_batch={} inst={} '
                      'effective_batch={} batches={} pair_weight={:.3f} tau={:.3f}'.format(
                          epoch,
                          int(cmbs_stats.get('common_ids', 0)),
                          int(cmbs_stats.get('unique_used_ids', 0)),
                          int(cmbs_stats.get('ids_per_batch', 0)),
                          int(cmbs_stats.get('instances', 0)),
                          int(cmbs_stats.get('effective_batch', 0)),
                          int(cmbs_stats.get('batches', 0)),
                          float(trainer.cmbs_pair_weight),
                          float(trainer.cmbs_pair_tau),
                      ))
            else:
                print('[CMBS][SKIP] epoch={} common_ids={} min_common={} reason={}'.format(
                    epoch,
                    int(cmbs_stats.get('common_ids', 0)),
                    int(getattr(args, 'cmbs_min_common_ids', 64)),
                    cmbs_stats.get('reason', 'unknown'),
                ))

        if cmbs_active:
            train_loader_ir = get_ordered_train_loader_ir(
                dataset_ir, trainset_ir_for_loader, transform_thermal, train_iters_target)
            train_loader_rgb = get_ordered_train_loader_rgb(
                dataset_rgb, trainset_rgb_for_loader, train_transformer_rgb,
                train_transformer_rgb1, train_iters_target)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage2-resume-full-state', type=str, default='',
                        help='Resume Stage-2 from a full-state handoff checkpoint, including optimizer, scheduler, RNG, and best-score state.')
    ...
    parser.add_argument('--stage2-only', action='store_true',
                        help='Compatibility mode: start at cmlabel from a model-only 20model_best checkpoint. Prefer full-chain or --stage2-resume-full-state for v36.')
    parser.add_argument('--stage2-init', type=str, default='',
                        help='Model-only checkpoint used with --stage2-only.')
    parser.add_argument('--enable-cmbs', action='store_true',
                        help='Enable v36 Stage-2 cross-modal identity-balanced synchronized IR/RGB batches.')
    parser.add_argument('--cmbs-num-instances', type=int, default=8,
                        help='Number of samples per shared identity and per modality in CMBS ordered batches.')
    parser.add_argument('--cmbs-min-common-ids', type=int, default=64,
                        help='Minimum IR/RGB shared pseudo-label identities required before CMBS is activated.')
    parser.add_argument('--cmbs-pair-weight', type=float, default=0.20,
                        help='Weight for the batch-level IR<->RGB multi-positive loss used with CMBS.')
    parser.add_argument('--cmbs-pair-tau', type=float, default=0.07,
                        help='Temperature for the batch-level IR<->RGB multi-positive loss.')
