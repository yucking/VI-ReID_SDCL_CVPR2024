...
"""
SDCL v39: EMA-teacher clustering.

The student keeps the original SDCL training objective.  A separate EMA teacher
is used only to extract features for DBSCAN/CRA pseudo-label generation, then
updated from the student after each epoch.  This targets pseudo-label topology
stability without adding hard cross-modal positive pairs.
"""
import argparse
...
def main_worker_stage2(args,log_name):
    ...
    teacher_model = None
    if getattr(args, 'enable_ema_teacher', False):
        teacher_model = make_model(cfg, num_class=0, camera_num=0, view_num=0)
        teacher_model.cuda()
        ...
        print('[EMA-T] enabled decay={:.5f} start_epoch={} cluster_only=True'.format(
            float(getattr(args, 'ema_teacher_decay', 0.999)),
            int(getattr(args, 'ema_teacher_start', 0))))
    ...
    def update_ema_teacher(teacher, student, decay):
        ...
        for key, teacher_value in teacher_state.items():
            ...
            if torch.is_floating_point(teacher_value):
                teacher_value.mul_(float(decay)).add_(student_value, alpha=1.0 - float(decay))
            else:
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch == trainer.cmlabel) :# 30 在特定轮次执行特定操作
            ...
            if teacher_model is not None:
                teacher_model.load_state_dict(model.state_dict())
                teacher_model.eval()
                print('[EMA-T] reset teacher from Stage-1 handoff checkpoint at epoch {}'.format(epoch))
        
        with torch.no_grad():
            cluster_model = model
            if (
                teacher_model is not None
                and epoch >= int(getattr(args, 'ema_teacher_start', 0))
            ):
                cluster_model = teacher_model
            cluster_source = 'ema_teacher' if cluster_model is teacher_model else 'student'
            print('[EMA-T] epoch={} clustering_source={}'.format(epoch, cluster_source))
            ...
            features_rgb, features_rgb_s = extract_features(cluster_model, cluster_loader_rgb, print_freq=50,mode=1)
            ...
            features_ir, features_ir_s = extract_features(cluster_model, cluster_loader_ir, print_freq=50,mode=2)
...
############## 共享层
        ...
        if teacher_model is not None:
            update_ema_teacher(teacher_model, model, float(getattr(args, 'ema_teacher_decay', 0.999)))
            teacher_model.eval()
            print('[EMA-T] epoch={} updated decay={:.5f}'.format(
                epoch, float(getattr(args, 'ema_teacher_decay', 0.999))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--enable-ema-teacher', action='store_true',
                        help='Use an EMA teacher only for clustering/CRA feature extraction.')
    parser.add_argument('--ema-teacher-decay', dest='ema_teacher_decay', type=float, default=0.999,
                        help='EMA decay for teacher model update after every epoch.')
    parser.add_argument('--ema-teacher-start', dest='ema_teacher_start', type=int, default=0,
                        help='Earliest epoch to use EMA teacher for clustering.')
    main()
